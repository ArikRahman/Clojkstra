...
(defn- page-for-route [handler]
  (case handler
    ...
    [:div {:class "flex flex-col items-center justify-center py-32 gap-4 text-center"}
     ...
     [:h1 {:class "text-3xl font-bold text-rp-text"} "404 — Page Not Found"]
     [:p  {:class "text-rp-muted text-sm max-w-xs"}
...
(defn- loading-overlay []
  (when @(rf/subscribe [::subs/loading?])
    [:div
     {:class "fixed inset-0 z-50 flex items-center justify-center bg-rp-base/80 backdrop-blur-sm"
      :aria-label "Loading"
      :role "status"}
...
(defn- nav-link [{:keys [handler label active]}]
  [:a
   {:class    (str "px-3 py-2 rounded-md text-sm font-medium transition-colors duration-150 "
                   (if active
                     "bg-rp-overlay text-rp-iris"
                     "text-rp-subtle hover:text-rp-text hover:bg-rp-overlay"))
...
(defn- app-navbar [app-name current-route]
  [:nav {:class "bg-rp-surface border-b border-rp-highlight-high sticky top-0 z-50"}
   [:div {:class "max-w-5xl mx-auto px-4 sm:px-6 lg:px-8"}
    [:div {:class "flex items-center justify-between h-16"}
     [:a {:class    "text-xl font-bold text-rp-iris tracking-tight"
          :href     "#/"
          :on-click (fn [e]
                      (.preventDefault e)
...
(defn- app-footer [app-name version]
  [:footer {:class "mt-auto border-t border-rp-highlight-high bg-rp-surface"}
   [:div {:class "max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-6
                  flex flex-col sm:flex-row items-center justify-between gap-3"}
    [:p {:class "text-xs text-rp-muted"}
     (str "© " (.getFullYear (js/Date.)) " " app-name " " version
          " — ClojureScript + re-frame starter template")]
    [:div {:class "flex items-center gap-4"}
     [:a {:class  "text-xs text-rp-muted hover:text-rp-iris transition-colors"
...
(defn- skip-link []
  [:a {:class "sr-only focus:not-sr-only focus:fixed focus:top-2 focus:left-2
               focus:z-50 focus:px-4 focus:py-2 focus:bg-rp-iris focus:text-rp-base
...
(defn app-root []
  (let [app-name      @(rf/subscribe [::subs/app-name])
    ...
    [:div {:class "min-h-screen flex flex-col bg-rp-base text-rp-text font-sans antialiased"}
     [skip-link]
