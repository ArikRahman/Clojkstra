(ns clojkstra.app.views
  ...
  (:require
   ...
   [clojkstra.app.pages.datascript :as datascript]))
...
(def ^:private nav-links
  [{:handler :home  :label "Home"}
   ...
   {:handler :datascript-demo :label "DataScript"}])
...
(defn- nav-link [{:keys [handler label active]}]
  [:a
   {:class    (str (:base theme/nav-link) " "
                   (if active (:active theme/nav-link) (:inactive theme/nav-link)))
...
(defn- app-navbar [app-name current-route]
  [:nav {:class (:outer theme/navbar)}
   [:div {:class "max-w-5xl mx-auto px-4 sm:px-6 lg:px-8"}
    [:div {:class "flex items-center justify-between h-16"}
     [:a {:class    (:brand theme/navbar)
          :href     "#/"
          :on-click (fn [e]
                      (.preventDefault e)
                      (rf/dispatch [::events/set-route {:handler :home :route-params {}}]))}
      app-name]
     [:div {:class (:links-row theme/navbar)}
