  ...
  "Semantic component style tokens — Layer 2 of the Clojkstra theme system.
   ...
   Architecture:
     Layer 1 — CSS custom properties (:root block in index.html)
               Kami palette values as --kami-* vars.
...
(def btn
  {:primary   "bg-accent text-surface hover:bg-accent-hi focus:ring-accent"
   ...
   :danger    "bg-danger text-surface hover:opacity-90 focus:ring-danger"})
...
(def card
  {:outer  "bg-surface rounded-xl border border-edge overflow-hidden"
   ...
   :title  "text-base font-semibold text-on-base font-serif"
   :sub    "mt-1 text-sm text-muted font-sans"
   :body   "px-6 py-4 font-serif"
...
(def card-item
  {:outer       "bg-surface border border-edge rounded-xl transition-all duration-150 hover:border-accent hover:bg-overlay"
   ...
   :heading     "font-semibold text-on-base text-sm mb-1 font-serif"
   :body        "text-sm text-muted leading-relaxed font-serif"})
...
(def alert-base
  "flex gap-3 p-4 rounded-lg border font-serif")
...
(def page-title
  {:heading  "text-3xl font-medium text-on-base tracking-tight font-serif"
   :subtitle "mt-2 text-base text-muted font-serif"})
...
(def divider
  {:line  "w-full border-t border-edge"
   :label "px-3 bg-base text-sm text-muted font-sans"
...
(def notification
  {:outer   "flex items-start gap-3 bg-surface border border-edge rounded-lg shadow-sm px-4 py-3 max-w-sm w-full"
   :message "flex-1 text-sm text-on-base font-serif"
...
(def input
  "w-full rounded-lg border border-edge bg-surface text-on-base px-3 py-2 text-sm
   focus:outline-none focus:ring-1 focus:ring-accent focus:border-accent
   transition placeholder-muted font-serif")

(def label
  "block text-xs font-medium text-subtle mb-1 font-sans tracking-wide uppercase")
...
(def navbar
  {:outer     "bg-surface border-b border-edge sticky top-0 z-50"
   :brand     "text-xl font-medium text-accent tracking-tight font-serif"
   :links-row "flex items-center gap-1"})

(def nav-link
  {:base     "px-3 py-2 rounded-md text-sm font-medium transition-colors duration-150 font-sans"
...
(def footer
  {:outer     "mt-auto border-t border-edge bg-surface"
   :copyright "text-xs text-muted font-sans"
   :link      "text-xs text-muted hover:text-accent transition-colors font-sans"})
...
(def shell
  {:root    "min-h-screen flex flex-col bg-base text-on-base font-serif antialiased"
...
(def skip-link
  "sr-only focus:not-sr-only focus:fixed focus:top-2 focus:left-2
   focus:z-50 focus:px-4 focus:py-2 focus:bg-accent focus:text-surface
   focus:rounded-lg focus:text-sm focus:font-medium font-sans")
...
(def page-404
  {:heading "text-3xl font-medium text-on-base font-serif"
   :body    "text-muted text-sm max-w-xs font-serif"})
...
(def table
  {:thead-row "border-b border-edge"
   :th        "text-left py-2 pr-4 font-semibold text-subtle font-sans text-xs uppercase tracking-wide"
   :tbody-row "border-b border-selection hover:bg-overlay transition-colors"
   :td-code   "text-xs text-accent font-mono"
   :td-muted  "py-2.5 text-muted font-serif"})
...
(def step-badge
  {:badge "flex-shrink-0 w-7 h-7 rounded-full bg-overlay text-accent text-sm font-medium flex items-center justify-center font-sans"
   :text  "text-sm text-subtle leading-relaxed pt-0.5 font-serif"})
...
(def feature-panel
  {:outer "rounded-lg bg-overlay border border-accent p-4"
   :text  "text-sm text-info-hi font-serif"})
...
(def detail-panel
  "rounded-lg border border-edge bg-overlay p-4 text-sm text-subtle leading-relaxed font-serif")
