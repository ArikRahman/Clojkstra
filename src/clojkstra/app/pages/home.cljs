...
(defn- hero []
  [:header {:class "py-20 text-center flex flex-col items-center gap-5"}
   ...
   [:h1 {:class (:heading theme/page-title)}
    "Clojkstra"]
   [:p {:class "text-xl text-subtle max-w-xl leading-relaxed"}
    "A ClojureScript + React (via Reagent + Re-frame) template designed for bootstrapping.
     Build new SPAs by forking, building, and thriving in a batteries included system."]
...
(defn- stack-section []
  [:section {:aria-labelledby "stack-heading"}
   [:h2 {:id    "stack-heading"
         :class "text-2xl font-bold text-on-base mb-6 text-center"}
    "The Stack"]
   [:div {:class "grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4"}
    (for [{:keys [name desc]} stack]
      ^{:key name}
      [:div {:class (str (:outer theme/card-item) " p-5")}
       [:p {:class (:heading theme/card-item)} name]
       [:p {:class (:body theme/card-item)} desc]])]])
  ...
  [:section {:aria-labelledby "principles-heading"}
   ...
   [:div {:class "grid grid-cols-1 md:grid-cols-2 gap-5"}
    (for [{:keys [icon title body]} principles]
      ^{:key title}
      [:div {:class (str (:outer-flex theme/card-item) " p-6")}
       [:span {:class "text-2xl flex-shrink-0 mt-0.5" :aria-hidden "true"} icon]
       [:div
        [:h3 {:class (:heading theme/card-item)} title]
        [:p  {:class (:body theme/card-item)} body]]])]])
   ...
   [:ol {:class "flex flex-col gap-4 mt-1"}
    (for [[n step] (map-indexed vector
      ...
      [:li {:class "flex gap-4 items-start"}
       [:span {:class (:badge theme/step-badge)} (inc n)]
       [:p   {:class (:text theme/step-badge)} step]])]])
