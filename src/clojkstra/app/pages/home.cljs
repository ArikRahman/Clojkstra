...
(defn- hero []
  [:header {:class "py-20 text-center flex flex-col items-center gap-5"}
   ...
   [:h1 {:class "text-5xl font-bold text-rp-text tracking-tight"}
    "Clojkstra"]
   [:p {:class "text-xl text-rp-subtle max-w-xl leading-relaxed"}
...
(defn- stack-section []
  [:section {:aria-labelledby "stack-heading"}
   [:h2 {:id    "stack-heading"
         :class "text-2xl font-bold text-rp-text mb-6 text-center"}
    "The Stack"]
   [:div {:class "grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4"}
    (for [{:keys [name desc]} stack]
      ^{:key name}
      [:div {:class "bg-rp-surface border border-rp-highlight-high rounded-xl p-5
                     hover:border-rp-iris hover:bg-rp-overlay
                     transition-all duration-150"}
       [:p {:class "font-semibold text-rp-text text-sm mb-1"} name]
       [:p {:class "text-sm text-rp-muted leading-relaxed"} desc]])]])
  ...
  [:section {:aria-labelledby "principles-heading"}
   ...
   [:div {:class "grid grid-cols-1 md:grid-cols-2 gap-5"}
    (for [{:keys [icon title body]} principles]
      ^{:key title}
      [:div {:class "bg-rp-surface border border-rp-highlight-high rounded-xl p-6 flex gap-4
                     hover:border-rp-iris transition-all duration-150"}
       [:span {:class "text-2xl flex-shrink-0 mt-0.5" :aria-hidden "true"} icon]
       [:div
        [:h3 {:class "font-semibold text-rp-text text-sm mb-1"} title]
        [:p  {:class "text-sm text-rp-muted leading-relaxed"} body]]])]])
   ...
   [:ol {:class "flex flex-col gap-4 mt-1"}
    (for [[n step] (map-indexed vector
      ...
      [:li {:class "flex gap-4 items-start"}
       [:span {:class "flex-shrink-0 w-7 h-7 rounded-full bg-rp-overlay text-rp-iris
                        text-sm font-bold flex items-center justify-center"}
        (inc n)]
       [:p {:class "text-sm text-rp-subtle leading-relaxed pt-0.5"} step]])]])
