    ...
    [ui/card {:title    "Local State Demo"
     ...
     [:div {:class "flex flex-col gap-4"}
      [:div {:class "flex gap-3 items-end"}
       [:div {:class "flex-1"}
        [:label {:class "block text-sm font-medium text-rp-subtle mb-1"
                 :for   "example-input"}
         "Type something"]
        [:input {:id          "example-input"
                 :class       "w-full rounded-lg border border-rp-highlight-high bg-rp-overlay text-rp-text px-3 py-2 text-sm
                                focus:outline-none focus:ring-2 focus:ring-rp-iris
                                focus:border-transparent transition placeholder-rp-muted"
                 :type        "text"
                 :value       input-value
    ...
    [ui/card {:title    "Feature Flag Demo"
     ...
     [:div {:class "flex flex-col gap-3"}
      [:div {:class "flex items-center gap-3"}
       [:span {:class "text-sm text-rp-subtle"} ":example-feature is currently"]
       (if enabled?
         [ui/badge {:label "enabled" :variant :success}]
         [ui/badge {:label "disabled" :variant :danger}])]
      (when enabled?
        [:div {:class "rounded-lg bg-rp-overlay border border-rp-iris p-4"}
         [:p {:class "text-sm text-rp-foam"}
...
(defn- toggle-detail []
  (let [open? (:show-detail? @local-state)]
    [:div {:class "flex flex-col gap-3"}
     ...
     (when open?
       [:div {:class "rounded-lg border border-rp-highlight-high bg-rp-overlay p-4 text-sm text-rp-subtle
                      leading-relaxed"}
