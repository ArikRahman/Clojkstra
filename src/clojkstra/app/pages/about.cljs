...
(defn- file-map-section []
  [ui/card {:title    "File Map"
   ...
   [:table {:class "w-full text-sm mt-1"}
    [:thead
     [:tr {:class "border-b border-rp-highlight-high"}
      [:th {:class "text-left py-2 pr-4 font-semibold text-rp-subtle w-52"} "File"]
      [:th {:class "text-left py-2 pr-4 font-semibold text-rp-subtle w-28"} "Kind"]
      [:th {:class "text-left py-2 font-semibold text-rp-subtle"}           "Purpose"]]]
    [:tbody
     (for [{:keys [file kind desc]} file-map]
       ^{:key file}
       [:tr {:class "border-t border-rp-highlight-high hover:bg-rp-overlay transition-colors"}
        [:td {:class "py-2.5 pr-4"}
         [:code {:class "text-xs text-rp-iris font-mono"} file]]
        [:td {:class "py-2.5 pr-4"}
         [kind-badge kind]]
        [:td {:class "py-2.5 text-rp-muted"} desc]])]]])
...
(defn- what-section [app-name version]
  [ui/card {:title (str "What is " app-name "?")}
   [:div {:class "flex flex-col gap-3 text-sm text-rp-subtle leading-relaxed"}
    [:p
    ...
    [:p
     ...
     [:span {:class "text-rp-text font-medium"} "app-db"]
     " holds all state, "
     [:span {:class "text-rp-text font-medium"} "events"]
     " are the only way to change it, "
     [:span {:class "text-rp-text font-medium"} "subscriptions"]
     " are the only way views read it, and "
     [:span {:class "text-rp-text font-medium"} "effects"]
