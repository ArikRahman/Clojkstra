...
(def ^:private file-map
  [{:file "core.cljs"          :kind :framework :desc "Entry point — init, hot-reload, mount"}
   {:file "db.cljs"            :kind :framework :desc "DataScript schema and seeded default state"}
   ...
   {:file "theme.cljs"         :kind :framework :desc "Semantic component style token maps"}
...
;;      │
;;      ▼
;; DataScript (immutable database — the primary source of truth)
;;      │
;;      ▼
...
(defn- file-map-section []
  [ui/card {:title    "File Map"
   ...
   [:table {:class "w-full text-sm mt-1"}
    [:thead
     [:tr {:class (:thead-row theme/table)}
      [:th {:class (str (:th theme/table) " w-52")} "File"]
      [:th {:class (str (:th theme/table) " w-28")} "Kind"]
      [:th {:class (:th theme/table)}               "Purpose"]]]
    [:tbody
     (for [{:keys [file kind desc]} file-map]
       ^{:key file}
       [:tr {:class (:tbody-row theme/table)}
        [:td {:class "py-2.5 pr-4"}
         [:code {:class (:td-code theme/table)} file]]
        [:td {:class "py-2.5 pr-4"}
         [kind-badge kind]]
        [:td {:class (:td-muted theme/table)} desc]])]]])
...
(defn- what-section [app-name version]
  [ui/card {:title (str "What is " app-name "?")}
   [:div {:class "flex flex-col gap-3 text-sm text-subtle leading-relaxed"}
    [:p
    ...
    [:p
     ...
     [:span {:class theme/emphasis} "datascript"]
     " manages data in the app, "
     [:span {:class theme/emphasis} "events"]
     " are the only way to change it, "
     [:span {:class theme/emphasis} "subscriptions"]
     " are the only way views read it, and "
     [:span {:class theme/emphasis} "effects"]
     ...
     [:header {:class "flex flex-col gap-3"}
      [ui/page-title
       {:title    (str "About " app-name)
        :subtitle "Clojkstra · cloak-straw · [ˈkloʊkstrɔ]: Portmanteau of Clojure and Dijkstra"}]
